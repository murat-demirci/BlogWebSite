                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (userListDto.ResultStatus === 0) {
                                ...
                                <button class="btn btn-warning btn-sm btn-asign" data-id="${user.Id}"><span class="fas fa-user-shield"></span></button>
                    ...
                    if (isValid) {
                        ...
                        const newTableRow = dataTable.row.add([
                            ...
                            `
                                ...
                                <button class="btn btn-warning btn-sm btn-asign" data-id="${userAddAjaxModel.UserDto.User.Id}"><span class="fas fa-user-shield"></span></button>
...
/* Ajax GET / Getting the _UserUpdatePartial as Modal Form starts from here. */
                    ...
                    if (isValid) {
                        ...
                        dataTable.row(tableRow).data([
                            ...
                            `
                                ...
                                <button class="btn btn-warning btn-sm btn-asign" data-id="${userUpdateAjaxModel.UserDto.User.Id}"><span class="fas fa-user-shield"></span></button>
    ...
    $(function () {
        ...
        /* Ajax POST / Updating a Role Asign starts from here */

        placeHolderDiv.on('click',
            '#btnAsign',
            function (event) {
                event.preventDefault();
                const form = $('#form-role-asign');
                const actionUrl = form.attr('action');
                const dataToSend = new FormData(form.get(0));
                $.ajax({
                    url: actionUrl,
                    ...
                    contentType: false,
                    success: function (data) {
                        const userRoleAsignAjaxModel = jQuery.parseJSON(data);
                        console.log(userRoleAsignAjaxModel);
                        const newFormBody = $('.modal-body', userRoleAsignAjaxModel.CommentUpdatePartial);
                        placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                        const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                        if (isValid) {
                            const id = userRoleAsignAjaxModel.UserDto.User.Id;
                            const tableRow = $(`[name="${id}"]`);
                            /*placeHolderDiv.find('.modal').modal('hide');*/
                            toastr.success(`${userRoleAsignAjaxModel.UserDto.Message}`, "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                            $('#validation-summary > ul > li').each(function () {
                                let text = $(this).text();
