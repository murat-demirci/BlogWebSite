...
namespace ProgrammersBlog.Mvc.Attributes
{
    ...
    public class ViewCountFilterAttribute : Attribute, IAsyncActionFilter
    {
        ...
        {
            var articleId = context.ActionArguments["articleId"];//detail action icin
            if (articleId is not null )
            {
                ...
                if (string.IsNullOrEmpty(articleValue))
                {
                    ...
                    var articlesService = context.HttpContext.RequestServices.GetService<IArticleService>();
                    await articlesService.IncreaseViewCountAsync(Convert.ToInt32(articleId));
                    await next();
                }
            }
            await next();
